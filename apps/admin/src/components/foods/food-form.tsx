...
import { Button, Input, Label, Textarea, Checkbox, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@repo/ui';
...
export function FoodForm({ food, categories, selectedCategoryId, isEditMode, onSubmit, onCancel, isPending }: Props): ReactNode {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="isPopular" className="cursor-pointer">인기메뉴 (BEST)</Label>
        </div>
        <div className="flex items-center gap-2">
          <Checkbox id="isNew" name="isNew" defaultChecked={food?.isNew ?? false} />
          <Label htmlFor="isNew" className="cursor-pointer">신메뉴 (NEW)</Label>
        </div>
      </div>
