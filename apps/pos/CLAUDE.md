# CLAUDE.md - POS (Electron)

> **IMPORTANT**: 이 파일은 POS 앱의 구조나 패턴이 변경될 때 반드시 함께 업데이트해야 합니다.
...
- Node.js 환경에서 실행
- BrowserWindow 생성 및 관리 (1920x1080)
- 시스템 API 접근 (파일, 프린터 등)
...
```typescript
import { cn, Button } from '@repo/ui'; // UI components & utilities
```
...
- 다국어 미지원 - 한국어 텍스트 직접 사용

## Kiosk vs POS 차이점

| 항목       | Kiosk           | POS              |
| ---------- | --------------- | ---------------- |
| 화면 크기  | 1080x1920 (세로) | 1920x1080 (가로) |
| 대상       | 고객용          | 직원용           |
| 다국어     | 지원 (ko, en, ja) | 미지원 (한국어만) |
| 용도       | 셀프 체크인     | 주문/결제 처리   |
...
- **DevTools 열림**: 개발 모드에서는 자동으로 열림
- **White screen**: Webpack 빌드 오류 확인, 콘솔 에러 확인
- **pnpm 호환성**: `.npmrc`에 `node-linker = hoisted` 설정 필요
